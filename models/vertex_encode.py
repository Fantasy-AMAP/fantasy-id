...
class StemNet(nn.Module):
    def __init__(self, emb_dim=128):
        ...
        self.pos_embedding = nn.Parameter(
            self._make_sine_position_embedding(emb_dim), requires_grad=False
        )
    ...
    def __init__(self, emb_dim, spiral_indices, up_transform):
        ...
        for idx in range(num_layer):
            self.de_layers.append(
                SpiralDeblock(
                    emb_dim, emb_dim, self.spiral_indices[num_layer - idx - 1]
                )
            )

        self.de_layers.append(SpiralConv(emb_dim, emb_dim, self.spiral_indices[0]))
...
def get_coarse_mesh_decoder(
    emb_dim=32, transform_fp="./assets/transform.pkl", down_degree=2
):
    dilation = [1, 1, 1, 1]
    seq_length = [9, 9, 9, 9]
    ...
    down_transform_list = [
        to_sparse(down_transform) for down_transform in tmp["down_transform"]
    ]
    up_transform_list = [
        to_sparse(up_transform) for up_transform in tmp["up_transform"]
    ]
...
class VertexSpiralNet(nn.Module):
    def __init__(
        self, EMB_DIM=128, num_v_coarse=314, transform_fp="./assets/transform.pkl"
    ):
        ...
        self.vtx_query = nn.Parameter(
            torch.zeros(1, num_v_coarse, EMB_DIM)
        )  # (1, Vc=314, 128)
        vtx_upsampler, down_transform_list = get_coarse_mesh_decoder(
            EMB_DIM, transform_fp
        )
        self.vtx_upsampler = vtx_upsampler
        self.down_transform_list = down_transform_list
        self.transformer = Transformer(
            dim=EMB_DIM, depth=6, heads=4, dim_head=EMB_DIM // 4, mlp_dim=EMB_DIM * 2
        )
    ...
    def depth_sin_pos_encoding(self, depth, temperature=10000, scale=2 * math.pi):
        ...
        pos_depth = torch.stack(
            [pos_depth[:, :, 0::2].sin(), pos_depth[:, :, 1::2].cos()], dim=3
        ).flatten(2)
...
class Conv1dBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conv1d = nn.Conv1d(
            inp_channels, out_channels, kernel_size, stride=stride, padding=padding
        )
...
class Conv1DReducer4(nn.Module):
    def __init__(self):
        ...
        self.conv_block3 = ResidualBlock1D(
            512, 1024, kernel_size=3, stride=2, padding=1
        )
        self.conv_block4 = ResidualBlock1D(
