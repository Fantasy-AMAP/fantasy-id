...
def albedo_ring_loss(texcode, ring_elements, margin, weight=1.0):
    ...
    for i in range(ring_elements - 1):
        for j in range(ring_elements - 1):
            ...
            tot_ring_loss = torch.add(
                tot_ring_loss, (torch.nn.functional.relu(margin + pd - nd).mean())
            )
            count += 1.0
...
def landmark_loss(predicted_landmarks, landmarks_gt, weight=1.0):
    ...
    if torch.is_tensor(landmarks_gt) is not True:
        ...
        real_2d = torch.cat(
            [landmarks_gt, torch.ones((landmarks_gt.shape[0], 68, 1)).cuda()], dim=-1
        )
    # real_2d = torch.cat(landmarks_gt).cuda()
...
def gradient_dif_loss(prediction, gt):
    ...
    diff = torch.mean((prediction_diff_x - gt_x) ** 2) + torch.mean(
...
def laplacian_hq_loss(prediction, gt):
    ...
    kernel_size = 3
    kernel = (
        get_laplacian_kernel2d(kernel_size).to(prediction.device).to(prediction.dtype)
    )
...
class VGG_16(nn.Module):
    ...
    def __init__(self):
        ...
        self.mean = (
            torch.Tensor(np.array([129.1863, 104.7624, 93.5940]) / 255.0)
            .float()
            .view(1, 3, 1, 1)
            .cuda()
        )
