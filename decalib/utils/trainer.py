    ...
    def __init__(self, model, config=None, device="cuda:0"):
        ...
        if self.cfg.train.write_summary:
            ...
            self.writer = SummaryWriter(
                log_dir=os.path.join(self.cfg.output_dir, self.cfg.train.log_dir)
            )

    def configure_optimizers(self):
        self.opt = torch.optim.Adam(
            self.E_flame.parameters(), lr=self.cfg.train.lr, amsgrad=False
        )
    ...
    def training_step(self, batch, batch_nb):
        ...
        if self.cfg.loss.id > 0.0:
            ...
            albedo_images = F.grid_sample(
                opdict["albedo"].detach(), opdict["grid"], align_corners=False
            )
        ...
        losses["shape_reg"] = (
            torch.sum(codedict["shape"] ** 2) / 2
        ) * self.cfg.loss.reg_shape
        losses["expression_reg"] = (
            torch.sum(codedict["exp"] ** 2) / 2
        ) * self.cfg.loss.reg_exp
        losses["tex_reg"] = (
            torch.sum(codedict["tex"] ** 2) / 2
        ) * self.cfg.loss.reg_tex
        losses["light_reg"] = (
        ...
        if self.cfg.model.jaw_type == "euler":
            ...
            losses["reg_jawpose_roll"] = (
                torch.sum(codedict["euler_jaw_pose"][:, -1] ** 2) / 2
            ) * 10.0
            losses["reg_jawpose_close"] = (
                torch.sum(F.relu(-codedict["euler_jaw_pose"][:, 0]) ** 2) / 2
            ) * 10.0
        ...
        for epoch in tqdm(range(start_epoch, self.cfg.train.max_epochs)):
            for step, batch in enumerate(tqdm(self.train_dataloader)):
                ...
                if self.global_step % self.cfg.train.checkpoint_steps == 0:
                    ...
                    torch.save(
                        model_dict, os.path.join(self.cfg.output_dir, "model" + ".tar")
                    )
