...
def augment_bbox(center, bbox_size, scale=[1.0, 1.0], trans_scale=0.0):
    ...
    trans_scale = (
        torch.rand([batch_size, 2], device=center.device) * 2.0 - 1.0
    ) * trans_scale
    center = center + trans_scale * bbox_size  # 0.5
    scale = (
        torch.rand([batch_size, 1], device=center.device) * (scale[1] - scale[0])
        + scale[0]
    )
...
def crop_tensor(
    ...
    src_pts = (
        torch.zeros([4, 2], dtype=dtype, device=device)
        .unsqueeze(0)
    ...
    trans_points_2d = torch.bmm(
        torch.cat(
            [
                points_2d,
                torch.ones(
                    [batch_size, n_points, 1], device=points.device, dtype=points.dtype
                ),
            ],
            dim=-1,
        ),
