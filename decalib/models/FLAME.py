...
class FLAME(nn.Module):
    ...
    def __init__(self, config):
        ...
        )  # (5023, 3)  val: [-0.19, 0.13] 猜测单位是米

        # The shape components and expression (basis)
        shapedirs = to_tensor(
            to_np(flame_model.shapedirs), dtype=self.dtype
        )  # (5023, 3, 300+100) shape & expression basis
        shapedirs = torch.cat(
            [
            ...
            2,
        )  # (5023, 3, n_shape + n_exp)
        self.register_buffer("shapedirs", shapedirs)

        # The pose components
        num_pose_basis = flame_model.posedirs.shape[-1]  # (5023, 3, 36) --> 36
        posedirs = np.reshape(
            flame_model.posedirs, [-1, num_pose_basis]
        ).T  # (5023, 3, 36) -> (36, 5023 * 3)
    ...
    def forward(
        ...
        lmk_faces_idx = torch.cat(
            [dyn_lmk_faces_idx, lmk_faces_idx], 1
        )  # the indices of the faces used to calculate the lmks
        lmk_bary_coords = torch.cat(
            [dyn_lmk_bary_coords, lmk_bary_coords], 1
        )  # barycentric coordinates that are used to interpolate the landmarks
...
class FLAMETex(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.tex_type == "BFM":
            ...
            texture_mean = tex_space[mu_key].reshape(
                1, -1
            )  # (1, 786432) == (1, 3, 512, 512) val: 0-1
            ...
            tex_space = np.load(tex_path)
            texture_mean = (
                tex_space[mu_key].reshape(1, -1) / 255.0
            )  # (1, 786432), 786432 = 3 * 512 * 512
        ...
        texture_basis = torch.from_numpy(texture_basis[:, :n_tex]).float()[
            None, ...
        ]  # (1, 786432, 199)
    ...
    def forward(self, texcode):
        ...
        texture = texture.reshape(texcode.shape[0], 512, 512, 3).permute(
            0, 3, 1, 2
        )  # (B, 786432) -> (B, 512, 512, 3) --> (B, 3, 512, 512)
        texture = F.interpolate(
            texture, [256, 256]
        )  # (B, 3, 512, 512) --> (B, 3, 256, 256)
